...
impl CACertMap {
    pub fn common_name(cert: &Certificate) -> Result<String, Error> {
        Self::common_name_from_str(cert.tbs_certificate.subject.to_string())
    }

    pub fn common_name_from_str(s: String) -> Result<String, Error> {
        let ss: Vec<&str> = s.split(',').collect();
        ss.iter()
            .find(|&&s| s.starts_with("CN=") || s.starts_with("OU="))
            .map(|name| name[3..].to_string())
            .ok_or(Error::BadX509Certificate)
    }
}
impl CACertMap {
    pub fn try_init() -> Result<Self, Error> {
        ...
        if !cl.errors.is_empty() || cl.certs.is_empty() {
            ...
            return Error::EmptySystemRootCACerts.into();
        }
    ...
    pub fn have_cert(&self, cn: String) -> Result<&Certificate, Error> {
        if let Some(cert) = self.root.get(&cn) {
            Ok(cert)
        } else {
            Error::CertificateWithCommonNameNotFound.into()
        }
    }
}
...
impl CertificateMsg {
    pub fn deserialize(deser: &mut DeSer) -> Result<(Self, Vec<u8>), Error> {
        ...
        if deser.peek_u8() != HandshakeType::Certificate as u8 {
            return Error::ExpectingCertificateVerifyMsg.into();
        };
        ...
        while cert_deser.available() > 0 {
            ...
            {
                return Error::PostDatedCertificate.into();
            }
            if time::SystemTime::now().duration_since(UNIX_EPOCH).unwrap()
                > cert.tbs_certificate.validity.not_after.to_unix_duration()
            {
                return Error::ExpiredCertificate.into();
            }
            ...
            match CACertMap::common_name(tls_serv_cert_list_root) {
                Ok(root_cn) => {
                    if let Ok(root_ca_cert) = sys_ca_certs.have_cert(root_cn) {
                        ...
                        return Error::MissingCARootCertificate.into();
                    }
                }
                _ => return Error::MissingCARootCertificate.into(),
            }
        ...
        Error::InvalidSystemRootCACert.into()
    }
    ...
    fn verify_rsa_signature(
        ...
        if "1.2.840.113549.1.1.11" == target_cert.signature_algorithm.oid.to_string() {
            ...
            let rsa_pub_key =
                RsaPublicKey::try_from(issuer_spki).map_err(|_| Error::BadSubjectPublicKeyInfo)?;
            ...
            Err(Error::UnsupportedDSASchemeInCertChainValidation)
        }?;
        Ok(())
    }
    ...
    fn verify_ecdsa_signature(
        ...
        if "1.2.840.10045.4.3.2" == target_cert.signature_algorithm.oid.to_string() {
            ...
            let sig = p256::ecdsa::DerSignature::try_from(target_cert_sig)
                .map_err(|_| Error::BadEcdsaP256Signature)?;
            let vk = p256::ecdsa::VerifyingKey::try_from(issuer_spki)
                .map_err(|_| Error::BadSubjectPublicKeyInfo)?;
            ...
            Err(Error::UnsupportedECDSASchemeInCertChainValidation)
        }?;
        Ok(())
    }
    ...
    fn verify_chain(certs: &[Certificate]) -> Result<(), Error> {
        for i in 0..certs.len() - 1 {
            ...
            if sig_alg_oid == "1.2.840.113549.1.1.11" || sig_alg_oid == "1.2.840.113549.1.1.12" {
                ...
                Error::UnsupportedSigSchemeInCertChainValidation.into()
            }?
        }
...
impl CertificateVerifyMsg {
    pub fn deserialize(deser: &mut DeSer) -> Result<(Self, Vec<u8>), Error> {
        ...
        if sig_len > (1 << 16) - 1 {
            return Error::BadCertificateSignatureLen.into();
        }
        let sig = deser.slice(len - 2);
        if sig.is_empty() {
            Error::MissingCertificateSignature.into()
        } else {
            Ok((
                Self {
                    // head,
    ...
    pub fn verify(&self, transcript_hash: Vec<u8>, cert: Certificate) -> Result<(), Error> {
        ...
        match self.sig_scheme {
            SignatureScheme::RsaPssRsaeSha256 => {
                let sig: rsa::pss::Signature = self
                    ...
                    .as_slice()
                    .try_into()
                    .map_err(|_| Error::BadRsaPssRsaeSha256Signature)?;
                // watch the two steps in obtaining the signature verification key
                let rsa_pub_key =
                    RsaPublicKey::try_from(spki).map_err(|_| Error::BadSubjectPublicKeyInfo)?;
